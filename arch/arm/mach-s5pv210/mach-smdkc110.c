...
{
	s3c_gpio_cfgpin(S5PV210_GPH1(2), S3C_GPIO_SFN(0x0f));	
}

static struct mcp251x_platform_data mcp251x_data = 
{
	.oscillator_frequency= 8 * 1000 * 1000,
	.board_specific_setup = &s3c6410_mcp2515_init,
...
/*	
	...
	if (err)
		...
		printk(KERN_ERR "#### request GPH2_1 \n");		
...
static void __init smdkc110_machine_init(void)
{
	...
	gpio_request(S5PV210_GPH2(1), "GPH2_1");	
#ifdef CONFIG_CAMERA_TVP5150_ONBOARD
	gpio_direction_output(S5PV210_GPH2(1), 1);
#else
	gpio_direction_output(S5PV210_GPH2(1), 0);
...
void usb_host_phy_init(void)
{
	...
	__raw_writel(__raw_readl(S3C_USBOTG_RSTCON) & ~(0x1<<4) & ~(0x1<<3),
			S3C_USBOTG_RSTCON);
	//boardcon,pull-up gsm/wcdma
	//set gph2con[1] = output = 0001
	unsigned long tmp;
	tmp = __raw_readl(S5PV210_GPH2_BASE);
	//printk("######GPH2CON before=%#x ##########\n",tmp);
	tmp &= ~(0xf << 4);
	tmp |= (0x1 << 4);
	__raw_writel(tmp, S5PV210_GPH2_BASE);
	//printk("######GPH2CON after=%#x #######\n",__raw_readl(S5PV210_GPH2_BASE));

	//set gph2DAT[1] = 0
	tmp = __raw_readl(S5PV210_GPH2_BASE + 0x4);
	//printk("######GPH2DAT before=%#x ########\n",tmp);
	tmp &= ~(0x1 << 1);
	__raw_writel(tmp, S5PV210_GPH2_BASE + 0x4);
	//printk("######GPH2DAT after=%#x #######\n",__raw_readl(S5PV210_GPH2_BASE + 0x4));

	/*RESET gsm/wcdma
	tmp = __raw_readl(S5PV210_GPH3_BASE);
	printk("######GPH3CON before=%#x ##########\n",tmp);
	tmp &= ~(0xf << 28);
	tmp |= (0x1 << 28);
	__raw_writel(tmp, S5PV210_GPH3_BASE);
	printk("######GPH3CON after=%#x #######\n",__raw_readl(S5PV210_GPH3_BASE));
	
	tmp = __raw_readl(S5PV210_GPH3_BASE + 0x4);
	printk("######GPH3DAT before=%#x ########\n",tmp);
	tmp |= (0x1 << 7);
	__raw_writel(tmp, S5PV210_GPH3_BASE + 0x4);
	printk("######GPH3DAT after11111=%#x #######\n",__raw_readl(S5PV210_GPH3_BASE + 0x4));
	mdelay(100);
	tmp &= ~(0x1 << 7);
	__raw_writel(tmp, S5PV210_GPH3_BASE + 0x4);
	printk("######GPH3DAT after=%#x #######\n",__raw_readl(S5PV210_GPH3_BASE + 0x4));
