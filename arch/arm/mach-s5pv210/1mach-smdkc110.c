...
static mcp251x_setup(void)
{
/*	s3c_gpio_cfgpin(S5PV210_GPB(2),0x02<<0); //miso
	s3c_gpio_cfgpin(S5PV210_GPB(0),0x02<<4); //clk
	s3c_gpio_cfgpin(S5PV210_GPB(3),0x02<<8); //mosi
	s3c_gpio_cfgpin(S5PV210_GPB(1),0x02<<12); //cs
    s3c_gpio_cfgpin(S5PV210_GPA0(3),0x07<<12); //rts0
	s3c_gpio_cfgpin(S5PV210_GPA0(2),0x01<<8);  //cts0

	s3c_gpio_setpull(S5PV210_GPA0(2),S3C_GPIO_PULL_DOWN);
	s3c_gpio_setpull(S5PV210_GPA0(3),S3C_GPIO_PULL_DOWN);*/
	...
	//s3c_gpio_cfgpin(S5PV210_GPH1(2), S3C_GPIO_SFN(0x0f));
	//s3c_gpio_cfgpin(S3C64XX_GPC(5), S3C_GPIO_SFN(1));
	//gpio_direction_output(S5PV210_GPH1(3),0);
	s3c_gpio_setpull(S5PV210_GPH1(3),0);
}
static struct mcp251x_platform_data mcp2515_data=
{
