...
static void s3c_nand_wait_enc(void)
{
	...
	#else
		if (readl(regs + NFECCSTAT) & NFSTAT_ECCENCDONE)
	#endif
			break;
		cond_resched();
	}
...
static void s3c_nand_wait_dec(void)
{
	...
	while (time_before(jiffies, timeo)) {
		...
		#else
		if (readl(regs + NFECCSTAT) & NFSTAT_ECCDECDONE)
		#endif
			break;
		cond_resched();
	}
...
static void s3c_nand_wait_ecc_busy_8bit(void)
{
	...
	while (readl(regs + S3C_NF8ECCERR0) & S3C_NFECCERR0_ECCBUSY) {
		...
		udelay(100);
	#else
	while (readl(regs + NFECCSTAT) & NFESTAT0_ECCBUSY) {
   #endif
	}
}
...
if (cur_ecc_mode == NAND_ECC_READ) {
	...
	/* clear 8/12/16bit ecc decode done */
	nfcont = readl(regs+NFECCSTAT);
	nfcont |= (1<<24);
	writel(nfcont, regs+NFECCSTAT);
...
int s3c_nand_correct_data_8bit(struct mtd_info *mtd, u_char *dat, u_char *read_ecc, u_char *calc_ecc)
{
	...
	nf8eccerr0 = readl(regs+NFECCSECSTAT);
	nf8eccerr1 = readl(regs+NFECCERL0);
	nf8eccerr2 = readl(regs+NFECCERL1);
	nf8eccerr3 = readl(regs+NFECCERL2);
	nf8eccerr4 = readl(regs+NFECCERL3);
	nfmlc8bitpt0 = readl(regs+NFECCERP0);
	nfmlc8bitpt1 = readl(regs+NFECCERP1);
