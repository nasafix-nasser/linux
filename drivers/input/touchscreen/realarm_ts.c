		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	{
		printk("realarm ts alibrate error: input command error,should be calibrate\n");	
		return count;
	}
	else
	{
	...
	{		
		if(Rdbuf[0])
		{
			...
			for(i=0; i<Rdbuf[0]; i++)
			{
				...
				{
					input_report_abs(tsdata->input, ABS_X, posx[0]);
					input_report_abs(tsdata->input, ABS_Y, posy[0]);
...
{ 	.driver =
		{
			.owner = THIS_MODULE,
			.name = "realarm_multi_touch_driver",
		}, 
	.probe = ts_i2c_probe, 
...
int ts_attach_adapter(void)
{
	...
	printk("ts_attach_adapter for realarm.\n");
	
	adapter = i2c_get_adapter(I2C_BUS_NUM);

	memset(&info, 0, sizeof(struct i2c_board_info));

	strlcpy(info.type, "realarm_ts", I2C_NAME_SIZE);
	...
	/*******************************class*************************************/
	ts_class = class_create(THIS_MODULE, "realarm_multi_touch_class");
	if (IS_ERR(ts_class)) {
...
static int __init i2c_ts_init(void)
{
	...
	printk("realarm multi touch init\n");
...
MODULE_DESCRIPTION("realarm multi-touch driver");
MODULE_LICENSE("GPL");
